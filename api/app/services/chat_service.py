...
class ChatService:
    def __init__(self, token: str, pocketbase_client: PocketBaseClient):
        self.token = token
    ...
    async def get_chats(self):
        chats = self.pocketbase_client.get_chats(self.token)
        return chats
    ...
    async def start_chat(self, message: dict, chat_id: str):
        ...
        self.pocketbase_client.add_message(self.token, message)

        # Check the existence of latest code
        source = self.pocketbase_client.get_source_metadata(self.token, chat_id)
        ...
        def on_message(assistant_message):
            ...
            self.pocketbase_client.add_message(self.token, assistant_message)
