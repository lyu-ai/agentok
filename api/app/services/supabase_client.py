...
from ..models import ApiKey, ApiKeyCreate, Chat, ChatCreate, Dataset, DatasetCreate, Document, DocumentCreate, Message, MessageCreate
...
class SupabaseClient:
    ...
    def get_user(self) -> User:
        try:
            ...
            user_response = self.supabase.rpc('get_user_by_id', {'user_id': self.user_id}).execute()
    ...
    def authenticate_with_tokens(self, access_token: str, refresh_token: Optional[str] = None) -> User:
        try:
            if not self.supabase_url or not self.supabase_service_key:
                raise Exception("Supabase URL or key not found in environment variables")
            temp_supabase = create_client(self.supabase_url, self.supabase_service_key)
            # Set the session in Supabase
            temp_supabase.auth.set_session(access_token=access_token, refresh_token=refresh_token or "dummy_refresh_token")
            ...
            if user_data:
                ...
                self.user_id = None
                raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Failed to authenticate with bearer token")
        except Exception as exc:
            logger.error(f"An error occurred: {exc}")
            raise
    ...
    def authenticate_with_apikey(self, apikey: str) -> User:
        try:
            ...
            if not api_key_response.data:
                raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Failed to authenticate with apikey")
            ...
            logger.error(f"An error occurred: {exc}")
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="An error occurred during authentication")
    ...
    def fetch_settings(self) -> Dict:
        try:
            response = self.supabase.table('users').select('settings').eq("user_id", self.user_id).execute()
            if response.data:
    ...
    def fetch_chats(self) -> List[Chat]:
        try:
            response = self.supabase.table('chats').select('*').eq("user_id", self.user_id).execute()
            if response.data:
    ...
    def fetch_chat(self, chat_id: str) -> Chat:
        try:
            response = self.supabase.table('chats').select('*').eq('id', chat_id).eq("user_id", self.user_id).execute()
            if response.data:
    ...
    def fetch_datasets(self) -> List[Dataset]:
        try:
          response = self.supabase.table('datasets').select("*").eq('user_id', self.user_id).execute()
    ...
    def fetch_dataset(self, dataset_id: str) -> Dataset:
        response = self.supabase.table('datasets').select("*").eq('id', dataset_id).eq('user_id', self.user_id).execute()
        if response.data and len(response.data) > 0:
            return Dataset(**response.data[0])
        else:
            raise HTTPException(
    ...
    def delete_dataset(self, dataset_id: str) -> Dict:
        response = self.supabase.table('datasets').delete().eq('id', dataset_id).eq('user_id', self.user_id).execute()
        if response.data:
    ...
    def fetch_documents(self, dataset_id: str) -> List[Document]:
        response = self.supabase.table('documents').select("*").eq('dataset_id', dataset_id).eq('user_id', self.user_id).execute()
        if response.data:
    ...
    def fetch_document(self, dataset_id: str, document_id: str) -> Document:
        response = self.supabase.table('documents').select("*").eq('document_id', document_id).eq('dataset_id', dataset_id).eq('user_id', self.user_id).execute()
        if response.data and len(response.data) > 0:
            return Document(**response.data[0])
        else:
            raise HTTPException(
    ...
    async def upload_document(self, dataset_id: str, filename: str, file_content: bytes) -> Document:
        try:
            if not filename:
                raise HTTPException(status_code=400, detail="Invalid file")
            print(colored(f"Uploading document to dataset {dataset_id} {filename}", 'light_blue'))
            # Read the file content
            md5_hash = hashlib.md5(file_content).hexdigest()
            file_extension = filename.split('.')[-1]
            unique_filename = f'{dataset_id}/{md5_hash}.{file_extension}'
            # Check if the file already exists
            existing_files = self.supabase.storage.from_('documents').list(path=unique_filename)
            ...
            if any(file['name'] == unique_filename for file in existing_files):
                ...
                print(colored(f"Deleting existing document: {unique_filename}", 'yellow'))
                self.supabase.storage.from_('documents').remove([unique_filename])
            self.supabase.storage.from_('documents').upload(unique_filename, file_content)
            print(colored(f"Uploaded document: {filename} to {unique_filename}", 'green'))
            return self.create_document(DocumentCreate(dataset_id=int(dataset_id), name=filename, path=unique_filename))
    ...
    def delete_document(self, dataset_id: str, document_id: str) -> Dict:
        response = self.supabase.table('documents').delete().eq('id', document_id).eq('dataset_id', dataset_id).eq('user_id', self.user_id).execute()
        if response.data:
    ...
    def update_document_status(self, doc_id, status):
        response = self.supabase.table('documents').update({'status': status}).eq('id', doc_id).execute()
        if not response.data:
            raise Exception(f"Error updating document status: {response}")

    def fetch_document_status(self, document_id):
        response = self.supabase.table('documents').select("status").eq('id', document_id).execute()
        if response.data:
    ...
    def fetch_source_metadata(self, chat_id: str) -> Dict:
        try:
            response = self.supabase.table('chats').select('*').eq('id', int(chat_id)).eq("user_id", self.user_id).execute()
            if response.data:
    ...
    def set_chat_status(self, chat_id: str, chat_status: Literal['ready', 'running', 'wait_for_human_input', 'completed', 'aborted', 'failed']):
        try:
          response = self.supabase.table('chats').update({'status': chat_status}).eq('id', chat_id).eq("user_id", self.user_id).execute()
          if response.data:
            return response.data
          else:
            return None
    ...
    def upload_image(self, iamge_path, image_data):
        try:
            ...
            # Check if the file already exists
            existing_files = self.supabase.storage.from_('documents').list(path=iamge_path)
            # TODO: This can be optimized to avoid multiple uploads of the same file
